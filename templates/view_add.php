  ...
  <form class="form form--add-lot container <?= isset($errors)&&(count($errors)>0)? 'form--invalid':'' ; ?>" action="add.php" method="post" enctype="multipart/form-data"> 
    ...
    <div class="form__container-two">
      <div class="form__item <?= isset($errors['name'])? 'form__item--invalid' :''  ; ?>"> 
        ...
        <span class="form__error"><?= isset($errors['name'])? $dict['name'].' '.$errors['name'] :''  ;?></span>
      </div>
      <div class="form__item <?= isset($errors['category'])? 'form__item--invalid': '' ; ?>">
        ...
        <span class="form__error"><?= isset($errors['category'])? $dict['category'].' '.$errors['category']:''  ;?></span>
      </div>
    </div>
    <div class="form__item form__item--wide <?= isset($errors['description'])? 'form__item--invalid':''  ?>">
      <label for="message">Описание</label>
      <textarea id="message" name="description" placeholder="<?= $lot['description'] ?? 'Напишите описание лота' ; ?>" value="<?= $lot['description'] ?? '' ; ?>"></textarea>
      <span class="form__error"><?= isset($errors['description'])? $dict['description'].' '.$errors['description'] :'' ?></span>
    </div>
    <div class="form__item form__item--file  <?= isset($errors['img'])? 'form__item--invalid':''  ?>">
      ...
      <span class="form__error"><?= isset($errors['img'])? $dict['img'].' '.$errors['img'] :'' ?></span>
    </div>
    <div class="form__container-three">
      <div class="form__item form__item--small <?= isset($errors['price'])? 'form__item--invalid':''  ; ?>">
        ...
        <span class="form__error"><?= isset($errors['price'])? $dict['price'].' '.$errors['price']:''  ;?></span>
      </div>
      <div class="form__item form__item--small <?= isset($errors['step'])? 'form__item--invalid':''  ; ?>">
        <label for="lot-step">Шаг ставки</label>
        <input id="lot-step" type="number" name="step" placeholder="0" value="<?= $lot['step'] ?? '' ; ?>">
        <span class="form__error"><?= isset($errors['step'])? $dict['step'].' '.$errors['step'] :'' ;?></span>
      </div>
      <div class="form__item <?= isset($errors['expiration'])? 'form__item--invalid':''  ; ?>">
        ...
        <span class="form__error"><?= isset($errors['expiration'])? $dict['expiration'].' '.$errors['expiration']:''  ;?></span>
      </div>
    </div>
